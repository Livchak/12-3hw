...
1. `Чтобы сделать первую букву заглавной, нужно обернуть запрос SUBSTRING_INDEX в UPPER(SUBSTRING....)1, 1). Тем самым мы говорим сделай первую букву большую.`
2. `Затем мы добавляем запрос LOWER(SUBSTRING...), 2), тем самым указываем все символы со второго маленькие`
3. `Оборачиваем оба эти запроса в CONCAT() чтобы получился один стобец`


```
SELECT 
	CONCAT(UPPER(SUBSTRING(SUBSTRING_INDEX(email, '@', 1), 1, 1)), LOWER(SUBSTRING(SUBSTRING_INDEX(email, '@', 1), 2))) AS USVER,
...
```
6. **Скриншот из консоли задание 6** 
<img src = "img/12-03-6.png" width = 80%>
